...
### ローカルで開くと表紙1枚しか出ない／ページ送りできない
`index.html` を **`file://` で直接ブラウザにドロップして開いている**のが原因。
本編は `<section data-markdown="draft.md">` が `draft.md` を fetch して組み立てるが、
`file://` ではブラウザのセキュリティ制限で fetch がブロックされるため、
本編が丸ごと空になり表紙だけが残る（公開URLでは正常に動く）。

ローカル確認は必ずHTTP経由で行う。リポジトリのルートで以下を実行し、
...
テンプレートには `file://` で開いたことを検知して画面に赤い警告を出すガードを入れてある。

### 表示が古いまま／`?print-pdf` が付いている
`?pdf` で開くとテンプレートのスクリプトがURLを `?print-pdf` に書き換える（reveal.jsの判定用）。
このURLのままだと全スライドが縦に並ぶ印刷用レイアウトになり、ページ送りが効かない。
クエリを外した通常のURLで開き直す。
