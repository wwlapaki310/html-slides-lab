...
- **`##` はスライドの先頭行に書く**：`theme.css` のタイトルバナーは「`<section>` の先頭要素が `h2`」のときだけ適用される。
  見出しの前に本文を書くとバナーにならない（スライド属性コメント `<!-- .slide: ... -->` は先頭に書いてよい）。
- **区切り `---` の前後には空行を入れる**：`index.html` 側の `data-separator="^
---
$"` が空行込みで判定しているため、
  空行がないと区切られずに1枚へ連結される。
- **画像・動画は実ファイルを `assets/` に置いてから参照する**：存在しないパスを書くとPDF化（Decktape）でも壊れたまま出力される。
